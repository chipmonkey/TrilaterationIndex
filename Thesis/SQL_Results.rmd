...
```{r sql_results, echo=FALSE, fig.cap="SQL Timings (s) - lower is better", fig.width=6, fig.height=3}
library(data.table)
library(ggplot2)
...
This initial boxplot shows the Trilateration performance against the st_distance function overall.  Note that we're on a log scale - some of the Trilateration improvements are 100 times faster in the best case, and overall notably faster in aggregate.  The pattern depends significantly on the sizes of $|Q|$, $|P|$, and $d$, so let's look at charts that split those out:
