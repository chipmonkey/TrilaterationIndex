...
In general, we split our current literature review into sevveral major sections:  1. Geospatial computational considerations, 2. Multilateration and 3. Nearest Neighbor Algorithms, plus a final minor section for ancillary references outside those two areas:
...
### Vincenty's Formula
Vincenty's Formula is a common non-spherical numeric solution to Earth-shaped ellipsoidal distance calculations however it "fails to converge for nearly antipodal points".[@Karney2013]  

### Karney's Formula
Improvements of the Vincenty algorithms to fix the convergence issues, were provided by Karney in 2013. [@Karney2013].  This is currently the state-of-the art implementation of precise geospatial distances.  
Implementations of this approach have already been implemented in Python (in the Geopy library), which we use in our Python implementations.[@geopy]
